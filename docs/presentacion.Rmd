...
- slope_of_peak_exercise_st_segment: pendiente del segmento ST en el electrocardiograma durante el pico del ejercicio físico. Esta medida indica la calidad del flujo sanguíneo al corazón, ya que una pendiente plana o descendente puede ser un indicador de enfermedad cardíaca.
  ...
  - `heart_disease_present = 1`: Pendiente ascendente (**Up-sloping**)
  - `heart_disease_present = 2`: Pendiente plana (**Flat**) 
  - `heart_disease_present = 3`: Pendiente descendente (**Down-sloping**)
...
- resting_ekg_results: resultados electrocardiográficos en reposo (valores 0,1,2).

- serum_cholesterol_mg_per_dl: colesterol sérico en mg/dl.

- oldpeak_eq_st_depression: oldpeak = depresión del ST inducida por el ejercicio en relación con el reposo, una medida de anormalidad en los electrocardiogramas. 
...
- exercise_induced_angina: dolor torácico inducido por el ejercicio (`0: Falso, 1: Verdadero`)
...
datatable(
  ...
  width = '100%',           # Hace que el widget ocupe el 100% del contenedor
)
```

## Variable objetivo para entrenamiento

```{r train_labels, echo=FALSE}
datatable(
  train_labels,
  options = list(
    scrollX = TRUE,         # Permite scroll horizontal si hay muchas columnas
...
train_labels |>
  ...
  theme_minimal() + 
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_fill_manual(values = c("chartreuse4", "tomato1"))
```
...
train_values |>
  ...
  geom_col() +
  labs(title = "Conteo por chest_pain_type") +
  theme_minimal() + 
  theme(plot.title = element_text(hjust = 0.5))
```
...
stby(
  data = train_values,
  INDICES = train_labels$heart_disease_present, # by Species
